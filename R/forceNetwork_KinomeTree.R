...
force_kinome_tree <- function(treedf)
{
  # reassing to new variable for absolutely no reason
  newtreedf = treedf
  
  #The following code is inefficient but this is how I approached the assignment:
  
  #1. Need to make a dataframe for the nodes that looks like this:
  ...
  #2. Need to make a dataframe for the edges or links between the nodes...
  ...
  # Solution: Concatenating identifiers to kinases,families,subfamilies and groups ('k_','f_','s_','g_')
  kinase_by_group_number$name <- paste0('k_',kinase_by_group_number$name)
  subfamily_by_group_number$name <- paste0('s_',subfamily_by_group_number$name)
  family_by_group_number$name <- paste0('f_',family_by_group_number$name)
  group$name <- paste0('g_',group$name)
  ...
  for(i in 1:dim(smaller_df)[1]){
    if(smaller_df$subfamily[i] == ""){
      #ids -> family
      source <- append(source, which(identified_node_df$name %in% paste0('k_',smaller_df$ids[i])) - 1)
      target <- append(target, which(identified_node_df$name %in% paste0('f_',smaller_df$family[i])) - 1)
    }else{
      #ids -> subfamily
      source <- append(source, which(identified_node_df$name %in% paste0('k_',smaller_df$ids[i])) - 1)
      target <- append(target, which(identified_node_df$name %in% paste0('s_',smaller_df$subfamily[i])) - 1)
      #subfamily -> family
      source <- append(source, which(identified_node_df$name %in% paste0('s_',smaller_df$subfamily[i])) - 1)
      target <- append(target, which(identified_node_df$name %in% paste0('f_',smaller_df$family[i])) - 1)
    }
    #family -> group
    source <- append(source, which(identified_node_df$name %in% paste0('f_',smaller_df$family[i])) - 1)
    target <- append(target, which(identified_node_df$name %in% paste0('g_',smaller_df$group[i])) - 1)
  }
  ...
  link_df <- data.frame(source,target,value=1)
  ...
  # Plot
  return(forceNetwork(Links = link_df, Nodes = node_df, zoom = TRUE, charge = -100,
               Source = "source", Target = "target",
               Value = "value", NodeID = "name",
               Group = "group_number", opacity = 0.8,
               fontSize = 100, legend = FALSE,height = "2000px"))
}
